...
describes what users get. Pick a format with `-o`, or let ali1688 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ali1688 <command> -o table   # aligned columns for reading
ali1688 <command> -o jsonl   # one JSON object per line, for piping
ali1688 <command> -o json    # a single JSON array
ali1688 <command> -o csv     # spreadsheet friendly
ali1688 <command> -o tsv     # tab-separated
ali1688 <command> -o url     # just the URL column
ali1688 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ali1688 <command> --fields id,title,url
```
...
```bash
ali1688 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ali1688 <command>            # a table, because this is a terminal
ali1688 <command> | wc -l    # JSONL, because this is a pipe
```

---
title: "Add a command"
description: "Model a real ali1688 record and expose it as a command, a route, and a tool at once."
weight: 10
---

The scaffold ships one example type, `page`. Real work means modelling the
records ali1688 actually serves. You do that in two files, and every surface
updates itself.

## 1. Model the record

In `ali1688/ali1688.go`, add a struct for the thing you are fetching
...
type Item struct {
    ...
    Owner string `json:"owner" kit:"link,kind=ali1688/user"` // an edge to another record
}
...
- `kit:"link,kind=<scheme>/<type>"` marks an outbound edge. It can point at
  another ali1688 type or at another site entirely, which is what lets a
  host walk the graph across tools.

## 2. Declare the operation

In `ali1688/domain.go`, add an input struct and a handler, then register
it in `Register`:
...
```bash
ali1688 item <id>                 # the command
curl 'localhost:7777/v1/item/<id>'      # the route, under serve
ant get ali1688://item/<id>       # the URI dereference, via a host
```
